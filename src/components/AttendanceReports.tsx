...
export const AttendanceReports: React.FC = () => {
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-300">
      {/* Top Banner */}
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 bg-white p-6 rounded-2xl border border-slate-200 shadow-xs print:hidden">
        <div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs sm:text-sm transition-all shadow-md shadow-emerald-600/20"
          >
          ...
          <button
            onClick={handlePrint}
            className="inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-slate-900 hover:bg-slate-800 text-white font-semibold text-xs sm:text-sm transition-all shadow-md"
          >
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 print:grid-cols-4">
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="text-slate-500 text-xs font-medium mb-1">Total Target Undangan</div>
          <div className="text-2xl font-bold text-slate-900 font-mono">{total}</div>
          <div className="text-[11px] text-slate-400 mt-1">100% dari basis data</div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="text-slate-500 text-xs font-medium mb-1">Peserta Hadir (Check-In)</div>
          <div className="text-2xl font-bold text-emerald-600 font-mono">{checkedIn}</div>
        ...
        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="text-slate-500 text-xs font-medium mb-1">Tidak Hadir / Belum Scan</div>
          <div className="text-2xl font-bold text-rose-500 font-mono">{absent}</div>
          <div className="text-[11px] text-rose-500 mt-1">{100 - rate}% Absen</div>
        </div>

        <div className="bg-white p-5 rounded-2xl border border-slate-200 shadow-xs">
          <div className="text-slate-500 text-xs font-medium mb-1">Status Verifikasi</div>
          <div className="text-lg font-bold text-blue-700">QR Terverifikasi</div>
          <div className="text-[11px] text-slate-400 mt-1">Aman & Terkunci</div>
        </div>
      </div>
      ...
      <div className="flex bg-slate-100 p-1.5 rounded-2xl gap-1 text-xs font-semibold w-fit print:hidden">
        <button
          onClick={() => setReportTab('table')}
          className={`px-4 py-2 rounded-xl flex items-center gap-1.5 transition-all ${
            reportTab === 'table' ? 'bg-white text-blue-700 shadow-xs' : 'text-slate-600 hover:text-slate-900'
          }`}
        >
          <FileText className="w-4 h-4" />
          <span>Tabel Detail Presensi</span>
        </button>
        <button
          onClick={() => setReportTab('charts')}
          className={`px-4 py-2 rounded-xl flex items-center gap-1.5 transition-all ${
            reportTab === 'charts' ? 'bg-white text-blue-700 shadow-xs' : 'text-slate-600 hover:text-slate-900'
          }`}
        >
